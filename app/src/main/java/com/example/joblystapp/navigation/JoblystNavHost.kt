...
import com.example.joblystapp.auth.MenuScreen
...
fun JoblystNavHost(activity: ComponentActivity, googleAuthUIClient: GoogleAuthUIClient) {
    ...
    NavHost(navController = navController, startDestination = "welcome1") {
        ...
        composable("menu") {
            MenuScreen(onBackPressed = { navController.popBackStack() }, navController)
        }
        composable("filter") {
            FilterScreen (navController)
        }
        composable("notif") {
            NotifScreen(onBackPressed = { navController.popBackStack() })
        }
        composable("saved") {
            SavedScreen(onBackPressed = { navController.popBackStack() })
        }
        composable("tag") {
            TagScreen(onBackPressed = { navController.popBackStack() })
        }
        composable("details1") {
